  ...
  <nav :class="['fixed top-0 left-0 right-0 z-50 transition-all duration-500 bg-white dark:bg-slate-900 shadow-lg shadow-slate-900/5 dark:shadow-slate-900/20 border-b border-slate-200/50 dark:border-slate-700/50']">
    ...
    <div v-if="mobileMenuOpen" class="lg:hidden bg-white/95 dark:bg-slate-900/95 backdrop-blur-2xl border-t border-slate-200/50 dark:border-slate-700/50">
      <div class="px-6 py-4 space-y-2">
        <router-link to="/" class="block py-2.5 text-slate-700 dark:text-slate-300 hover:text-sky-600 font-semibold transition-colors" @click="mobileMenuOpen = false">Home</router-link>
        <router-link to="/rooms" class="block py-2.5 text-slate-700 dark:text-slate-300 hover:text-sky-600 font-semibold transition-colors" @click="mobileMenuOpen = false">Rooms</router-link>
        <router-link to="/facilities" class="block py-2.5 text-slate-700 dark:text-slate-300 hover:text-sky-600 font-semibold transition-colors" @click="mobileMenuOpen = false">Facilities</router-link>
        <router-link to="/gallery" class="block py-2.5 text-slate-700 dark:text-slate-300 hover:text-sky-600 font-semibold transition-colors" @click="mobileMenuOpen = false">Gallery</router-link>
        <router-link to="/about" class="block py-2.5 text-slate-700 dark:text-slate-300 hover:text-sky-600 font-semibold transition-colors" @click="mobileMenuOpen = false">About</router-link>
        <router-link to="/contact" class="block py-2.5 text-slate-700 dark:text-slate-300 hover:text-sky-600 font-semibold transition-colors" @click="mobileMenuOpen = false">Contact</router-link>
        <button @click="toggleThemeAndClose" class="flex items-center gap-3 py-2.5 text-slate-700 dark:text-slate-300 hover:text-sky-600 font-semibold transition-colors w-full">
        ...
        <div v-if="isAuthenticated" class="pt-3 border-t border-slate-200 dark:border-slate-700">
          <div class="flex items-center gap-3 py-2.5">
            <div class="w-8 h-8 rounded-xl overflow-hidden bg-gradient-to-br from-teal-500 to-emerald-600 flex items-center justify-center text-white font-bold">
              <img v-if="profileImage" :src="profileImage" alt="Profile" class="w-full h-full object-cover" />
              <span v-else>{{ userInitial }}</span>
            </div>
            <p class="font-bold text-slate-800 dark:text-white">{{ userName }}</p>
          </div>
          <router-link to="/my-rooms" class="flex items-center gap-3 py-2.5 text-slate-700 dark:text-slate-300 hover:text-sky-600 font-semibold transition-colors" @click="mobileMenuOpen = false">
            <Building2 class="w-4 h-4" />
            My Rooms
          </router-link>
          <router-link to="/my-payments" class="flex items-center gap-3 py-2.5 text-slate-700 dark:text-slate-300 hover:text-sky-600 font-semibold transition-colors" @click="mobileMenuOpen = false">
             <CreditCard class="w-4 h-4" />
             My Payments
           </router-link>
           <router-link to="/profile-management" class="flex items-center gap-3 py-2.5 text-slate-700 dark:text-slate-300 hover:text-sky-600 font-semibold transition-colors" @click="mobileMenuOpen = false">
             <User class="w-4 h-4" />
             Profile Management
           </router-link>
        ...
        <router-link v-else to="/login" class="block py-2.5 px-6 mt-2 text-center bg-white dark:bg-slate-800 text-slate-800 dark:text-white rounded-xl font-bold border-2 border-slate-200 dark:border-slate-700 hover:border-sky-500 transition-colors" @click="mobileMenuOpen = false">Login / Sign Up</router-link>
      </div>
    </div>
  </nav>
</template>
