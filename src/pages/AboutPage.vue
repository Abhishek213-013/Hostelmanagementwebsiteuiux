        ...
        <AnimatedSection :delay="400">
          <div class="mb-20">
            ...
            <template v-else-if="teamMembers.length > 0">
              ...
              <!-- Teams -->
              <div v-for="team in teams" :key="team.id" class="mb-20 last:mb-0">
                <!-- Team Header -->
                <div v-if="teams.length > 1" class="mb-6">
                  <h3 class="text-2xl font-black text-teal-600 mb-1">{{ team.name }}</h3>
                  <p v-if="team.description" class="text-gray-600 dark:text-gray-400">{{ team.description }}</p>
                  <div class="w-16 h-1 bg-teal-600 rounded-full mt-3"></div>
                </div>
...
// Use composables
const { teams, teamMembers, loading: teamLoading, error: teamError, fetchAllTeamsWithMembers } = useTeam()
...
// Progressive fetch - Load critical content first, rest in background
async function fetchAllData() {
  ...
  try {
    ...
    // STEP 1: Load page sections (critical - needed for hero)
    await fetchPageSectionsData(2)
    console.log('✅ Page sections loaded')
    
    // STEP 2: Parse and show about section immediately, then hide loading
    ...
    await Promise.allSettled([
      fetchTestimonials().catch(e => console.warn('Testimonials unavailable:', e)),
      fetchRoomTypes().catch(e => console.warn('Room types unavailable:', e)),
      fetchFacilities().catch(e => console.warn('Facilities unavailable:', e)),
      fetchAllTeamsWithMembers().catch(e => console.warn('Team data unavailable:', e)),
    ])

    // STEP 4: Parse stats and remaining sections after background data arrives
